    ...
    $("body").click(function (e) {
      if (_this.inputActive) {
        ...
        setTimeout(function () {
          ...
          _this.$ul.slideUp(100);

          _this.updateList(_this.data);
    ...
    value: function setInputListeners() {
      ...
      this.$input.keyup(function (e) {
        ...
        if (e.which == keys.ENTER) {
          ...
          var li = _this5.$ul.find('li.selected');

          if (li) {
            var id = $(li).data('id') || _this5.filtered[0][_this5.accessors.id];

            _this5.setSelectedItem(id);

            _this5.$input.blur();

            setTimeout(function () {
              _this5.updateList(_this5.data);

              _this5.onSelected(id);
            }, 300);
          }
        } else if (e.which == keys.ARROW_UP || e.which == keys.ARROW_DOWN) {
          _this5.setCursor(e.which);

          _this5.$input.val(value);
        } else {
          if (value.trim() != '') {
            _this5.cursor = 0;

            var filtered = _this5.data.filter(function (c) {
              return c[_this5.accessors.name] && c[_this5.accessors.name].toLowerCase().startsWith(value.toLowerCase());
            });

            _this5.filtered = filtered.length > 0 ? filtered : _this5.filtered;

            _this5.updateList(_toConsumableArray(filtered));

            _this5.setSelectedClass(_this5.filtered[0]);

            var temp = _this5.filtered.length > 0 ? _this5.filtered[0] : _this5.selectedItem;

            _this5.$placeholder.text(temp[_this5.accessors.name]);

            _this5.$input.val(temp[_this5.accessors.name].substring(0, value.length));

            _this5.setInput(temp, true);
          } else {
            _this5.cursor = -1;
            ...
            _this5.setInput(_this5.selectedItem, true);
          }
        }
      });
    }
